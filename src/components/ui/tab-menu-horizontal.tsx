...
>(({ className, ...rest }, forwardedRef) => {
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        // base
        'group/tab-item h-12 text-label-sm text-text-base outline-none',
        'flex items-center justify-center gap-1.5',
        'transition duration-200 ease-out',
        // focus
        'focus:outline-none',
        // active
        'data-[state=active]:text-text-dark',
        className,
      )}
      {...rest}
    />
...
function TabMenuHorizontalArrowIcon<T extends React.ElementType>({
  ...
  return (
    <Component
      className={cn('size-5 text-text-base', className)}
      {...rest}
    />
  );
}
